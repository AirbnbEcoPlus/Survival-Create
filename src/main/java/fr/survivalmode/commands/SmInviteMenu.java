	...
	public boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args) {
		...
		String namePlayerSend = player.getName();
		 if(!cmd.getName().equalsIgnoreCase("scinvite")) return false;
		    if(args.length == 0) { 
		    	Inventory invinv = Bukkit.createInventory(null, 9, "ß8SurvivalCreateInvite");
				...
				invinv.setItem(6, switchworld);
					...
					ItemMeta switchworldM = switchworld.getItemMeta();
					switchworldM.setDisplayName("Mettre votre monde en priv√©");
					switchworld.setItemMeta(switchworldM);
					invinv.setItem(6, switchworld);
				}
		        ...
		        if (target.getWorld().getName().equalsIgnoreCase("world")) {
		        	Inventory invinvaccept = Bukkit.createInventory(null, 9, "ß8SurvivalCreateInviteDemande");
		        	...
		        	player.sendMessage("ß2Ce joueur n'est pas present dans le spawn du [GAMEMODE]");
